...
{
    [Fact]
    public async Task PhoneService_ValidatePhoneNumber()
    {
        var token = "api_token";
        ...
        var url = MockedHttpRequest.JoinUrl(config.BaseUrl.ToString(), "/phone/validate");
        ...
        var service = new PhoneService(mockedHttpRequest);

        var result = await service.ValidatePhoneNumber(new() { Request = null }, false);
